function [dpdd,M,lambda] = bias_sensitivity_alt(obj,results,Smu)
  % [dpdd,M,lambda] = obj.bias_sensitivity_alt(results,Smu)
  %
  ...
  %
  % obj     : QSN object
  % results : cell array of controller data converted from pyspinnet
  % 
  ...
  for run = 1:size(results,2)
    ...
    H = obj.H + diag(results{run}.x);
    [V,e] = eig(H);
    lambda{run} = diag(e);

    S = cellfun(@(x) results{run}.T * V' * x * V, Smu, 'UniformOutput', false);
    SINC = sinc((lambda{run} * ones(1,obj.N) - ones(obj.N,1) * lambda{run}') * results{run}.T / (2*pi));
    SUML = (lambda{run} * ones(1,obj.N) + ones(obj.N,1) * lambda{run}') * results{run}.T / 2;
    %outV = out_vec'*V;
    outV = V(results{run}.Out,:);
    %Vin  = V'*in_vec;
    Vin = V(results{run}.In,:)';
    inout= 2 * (outV' .* conj(Vin));
    for j = 1:obj.N
      SIN  = sin(SUML-lambda{run}(j) * results{run}.T);
      M{run,j} = SIN .* SINC;
      for s = 1:length(S)
