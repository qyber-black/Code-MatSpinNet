function A = LLL_reduction(A,delta)
  % A = LLL_reduction(A,delta) - LLL lattice reduction
  %
  % Perform LLL basis reduction.
  %
  %   A      - (input) square matrix whose columes are the basis vectors
  ...
  B(:,1) = A(:,1); % Set the first column of B as the same as the first column of A
  M(1) = dot(B(:,1), B(:,1));
  for i = 2:m
